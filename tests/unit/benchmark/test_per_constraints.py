...
async def test_resolve_profile_allows_per_constraints_for_sweep():
    profile = await resolve_profile(
        ...
        console=None,
        per_constraints={"max_seconds": [1, 2, 3]},
    )

    assert isinstance(profile, SweepProfile)
    assert profile.per_constraints == {"max_seconds": [1, 2, 3]}

@pytest.mark.smoke
@pytest.mark.asyncio
async def test_resolve_profile_rejects_per_constraints_for_non_sweep():
    with pytest.raises(ValueError, match="Per-strategy constraints are only supported"):
...
def test_sweep_profile_applies_per_constraints_sequence(monkeypatch):
    ...
    profile = SweepProfile(
        sweep_size=3,
        per_constraints={"max_seconds": [5, None, 15]},
    ...
    assert "max_seconds" not in captured[-1]
    assert captured[-1]["max_requests"] == 100

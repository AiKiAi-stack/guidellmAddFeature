...
def test_sweep_profile_strategy_constraints():
    ...
    profile = SweepProfile(
        ...
        constraints={
            "max_duration": {"type_": "max_duration", "max_duration": 60.0},
            "max_requests": [10, 100, 200, 300, 400],
            "max_errors": [None, 5, 10, 15, None],
        },
    )
    ...
    assert profile.strategy_constraints == {
        "max_requests": [10, 100, 200, 300, 400],
        "max_errors": [None, 5, 10, 15, None],
    }
    ...
    assert constraints_dict["max_duration"].max_duration == 60.0
    assert constraints_dict["max_requests"].max_num == 10
    assert "max_errors" not in constraints_dict  # Correctly disabled
    ...
    expected_async_max_errors = [10, 15, None]
    for i in range(3):
        strategy, constraints_dict = generator.send(mock_benchmark)
        assert isinstance(strategy, AsyncConstantStrategy)
        assert constraints_dict is not None
